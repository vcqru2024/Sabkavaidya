...
dnnModule.digitalAssets = function ($, $find, $telerik, dnnModal) {
    ...
    function showDialog(controlKey, params, width, height) {
        ...
        dnnModal.show(url, /*showReturn*/true, height, width, true, null);
    }
    ...
    function getContentServiceUrl() {
        if (contentServiceUrl == '') {
            contentServiceUrl = servicesFramework.getServiceRoot('CrossDigitalAssets') + 'ContentService/';
        }
    ...
    function uploadFiles() {
        //we need to modify here, to use our own upload control
        window.location.href = settings.fileUploadUrl;
        //pass fromManager parm, then hide the "return to my files" button in UserGeneral_LocalFileUpload control
        // showDialog(settings.fileUploadControl, { fromManager: "1" }, 950, 850);
        //hide the built-in upload control
