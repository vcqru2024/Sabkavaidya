...
namespace Cross.Modules.CP_Product.View
{
    ...
    public partial class AdList : CPModuleBase
    {
        /// <summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            LocalResourceFile = Localization.GetResourceFile(this, "AdList.ascx");
            ctlPagingControl.ResourceFile = Cross.Modules.CP_Product.Business.LocalUtils.ResourcesFile_Shared;
            if (Settings_List.Image.UseLightBoxEffect && Settings_List.Image.ShowImage)//注册图片直接弹出效果,仅对classified ads 有效
            {
        ...
        private List<ProductInfo> GetProducts_ExternalSearch_Basic(int pageIndex, ref int pageSize, ref int totalRecords)
        {
            ...
            else //查询模式的其他查询
            {
                ...
                if (!string.IsNullOrEmpty(Settings_List.General.GroupIdMapping))
                {
                    if (Request.QueryString[Settings_List.General.GroupIdMapping] != null && Utils.IsNumber(Request.Params[Settings_List.General.GroupIdMapping].ToString()))
                    {
                        groupId = Convert.ToInt32(Request.Params[Settings_List.General.GroupIdMapping]);
                    }
                }
    ...
    && Request.Form["S_UDT" + i.ToString() + "_Oper"] != null
     && Utils.IsNumber(Request.Form["S_UDT" + i.ToString() + "_ID"]))
        ...
        protected void grProduct_DataBinding(object sender, EventArgs e)
        {
            ...
            if (grProduct.Columns[12] != null)//Price
            {
                grProduct.Columns[12].Visible = Settings_List.Content.ShowPrice && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[13] != null)//PromoStart
            {
                grProduct.Columns[13].Visible = Settings_List.Content.ShowPromoStart && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[14] != null)//PromoEnd
            {
                grProduct.Columns[14].Visible = Settings_List.Content.ShowPromoEnd && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[15] != null)//CreatedDate
            {
                grProduct.Columns[15].Visible = Settings_List.Content.ShowCreatedDate;
            }

            if (grProduct.Columns[16] != null)//Featured
            {
                grProduct.Columns[16].Visible = Settings_List.Content.ShowFeaturedField;
            }


            if (grProduct.Columns[17] != null)//Summary
            {
                grProduct.Columns[17].Visible = Settings_List.Content.ShowSummary && CurrentType.FieldEnable_Summary;
            }
        }
