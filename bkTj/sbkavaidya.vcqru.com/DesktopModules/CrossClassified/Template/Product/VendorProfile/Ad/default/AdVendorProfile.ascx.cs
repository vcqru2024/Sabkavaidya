...
namespace Cross.Modules.CP_Product.View
{
    ...
    public partial class AdVendorProfile : CPModuleBase
    {
        /// <summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            LocalResourceFile = Localization.GetResourceFile(this, "AdVendorProfile.ascx");
            ctlPagingControl.ResourceFile = Cross.Modules.CP_Product.Business.LocalUtils.ResourcesFile_Shared;
            if (Settings_VendorProfile.Product_Image.UseLightBoxEffect && Settings_VendorProfile.Product_Image.ShowImage)//注册图片直接弹出效果,仅对classified ads 有效
            {
        ...
        protected string GetShortenSummary(string productId, string summary)
        {
            ...
            return length > 0 ?
                DotNetNuke.Common.Utilities.HtmlUtils.Shorten(summary, length, "..").Replace(Microsoft.VisualBasic.Constants.vbCrLf, "<br/>") :
                summary.Replace(Microsoft.VisualBasic.Constants.vbCrLf, "<br/>");
        }
        protected string GetNavigate(string productId, string productName)
        {
            if (Settings_VendorProfile.Product_Image.UseLightBoxEffect)//如果当前设置为点击小图打开大图
            {
                ProductInfo objProduct = ProductController.Get(Convert.ToInt32(productId));
                return GetRelativeUrlByFileId(objProduct.Image) == "" ? LocalUtils.DefaultPicture : GetRelativeUrlByFileId(objProduct.Image);
            }
        ...
        protected void grProduct_DataBinding(object sender, EventArgs e)
        {
            ...
            if (grProduct.Columns[10] != null)//Price
            {
                grProduct.Columns[10].Visible = Settings_VendorProfile.Product_Content.ShowPrice && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[11] != null)//PromoStart
            {
                grProduct.Columns[11].Visible = Settings_VendorProfile.Product_Content.ShowPromoStart && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[12] != null)//PromoEnd
            {
                grProduct.Columns[12].Visible = Settings_VendorProfile.Product_Content.ShowPromoEnd && CurrentType.FieldEnable_Price;
            }
            if (grProduct.Columns[13] != null)//CreatedDate
            {
                grProduct.Columns[13].Visible = Settings_VendorProfile.Product_Content.ShowCreatedDate;
            }

            if (grProduct.Columns[14] != null)//Featured
            {
                grProduct.Columns[14].Visible = Settings_VendorProfile.Product_Content.ShowFeaturedField;
            }


            if (grProduct.Columns[15] != null)//Summary
            {
                grProduct.Columns[15].Visible = Settings_VendorProfile.Product_Content.ShowSummary && CurrentType.FieldEnable_Summary;
            }
        }
